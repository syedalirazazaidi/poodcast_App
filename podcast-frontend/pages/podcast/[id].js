...
export default function PodCastView() {
  ...
  useEffect(async () => {
    const data = await axios.get("http://localhost:1337/api/podcasts/" + id);
    console.log(data?.data?.data.attributes.name, "DATA");
        ...
        <div style={{ width: "100%" }}>
          ...
          <div className={styles.podcastviewepisodescont}>
            ...
            <div className={styles.podcastviewepisodeslist}>
              {console.log(podcast, "T--TTT")}
              <p>{podcast.attributes.name}</p>
              {/* {console.log(podcast.data.attributes, ";;;;")} */}
              {/* <p>{data?.data?.data.attributes.name}</p> */}
              {/* {podcast?.attributes.map((episode, i) => (
