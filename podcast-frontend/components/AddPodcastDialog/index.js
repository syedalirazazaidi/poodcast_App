...
export default function AddPodcastDialog({ closeModal }) {
  ...
  async function savePodcast() {
    ...
    for (let index = 0; index < episodes.length; index++) {
      ...
      axios.get("http://localhost:1337/api/episodes").then((response) => {
        console.log(response, "RESPONCE");
      });
      console.log(episode, "lollipop");
      //   if (episode && episode.length > 0) {
      //     const result = await axios.post("http://localhost:1337/api/episodes", {
      //         episode: episode,
      //     });
      if (episode && episode.length > 0) {
        axios
          .post("http://localhost:1337/api/episodes", { episode: episode })
          .then((response) => {
            console.log(response, "RESPO--NCE");
          });
      }
    ...
    // add podcast
    const data = await axios.post("http://localhost:1337/api/podcasts", {
      name: podcastName,
